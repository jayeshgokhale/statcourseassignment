...
What we have done here is scaled our distribution close to Standard Normal Distribution (we believe so) so that we can compare with one. For reference we take a standard normal distribution of 1000 points. Now we plot with the reference standard normal distribution in X axis and our distribution (scaled). If that is close to a straight line, then we can safely conclude that our Distribution is Normal.
...
This graph is self evident. **We can safely conclude that the distribution is NORMAL**
